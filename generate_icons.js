...
if (require.main === module) {
  ...
  function generate_logo_svg_paths() {
    ...
    // Configuration
    const canvas_size_width = 400; // Reduced from 800 to be tighter
    const canvas_size_height = 250; // Reduced from 800 to be tighter
    const cx = canvas_size_width / 2;
    const cy = canvas_size_height / 2;
    const card_w = 350; // Reduced to fit tighter around content
    const card_h = 200; // Reduced to fit tighter
    ...
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg width="${canvas_size_width}" height="${canvas_size_height}" viewBox="0 0 ${canvas_size_width} ${canvas_size_height}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="cardGrad" x1="0%" y1="0%" x2="100%" y2="100%">
