<script lang="ts">
    ...
    function getMessageDetails(v: any): {
        role: string;
        content: string;
        reasoning?: string;
        tool_calls?: any[];
    ...
    $: isMessageList =
        isArray &&
        value.length > 0 &&
        value.every((item: any) => !!getMessageDetails(item));
</script>
...
<div class="font-mono text-xs leading-relaxed">
    ...
    {#if isPrimitive(value)}
        <div class="flex items-start">
            {#if keyName}
                <span class="mr-2" style="color: var(--vscode-link);"
                    >{keyName}:</span
                >
            {/if}
            <span
                style="color: {typeof value === 'string'
                    ? 'var(--vscode-success)'
                    : typeof value === 'number'
                      ? 'var(--vscode-link)'
                      : typeof value === 'boolean'
                        ? '#c586c0'
                        : 'var(--vscode-secondary-fg)'}"
            >
    ...
    {:else if isMessage && messageData}
        ...
        <div
            class="border border-gray-200 dark:border-gray-700 rounded my-1 bg-white dark:bg-gray-800"
        >
            <div
            ...
            >
                ...
                <span
                    class="font-bold text-xs uppercase tracking-wider px-1.5 py-0.5 rounded
                ...
                {#if messageData.role === "Tool" && messageData.metadata?.name}
                    <span
                        class="text-purple-600 dark:text-purple-400 text-[10px] font-mono border border-purple-200 dark:border-purple-800 px-1 rounded bg-purple-50 dark:bg-purple-900/20"
                    >
            ...
            {#if !isOpen}
                <div
                    class="px-3 py-2 text-gray-600 dark:text-gray-400 cursor-pointer"
                    on:click={toggle}
                    role="button"
                    tabindex="0"
                    on:keydown={(e) => e.key === "Enter" && toggle()}
                >
                    <div class="flex flex-wrap gap-2 mb-1">
                        {#if messageData.reasoning}
                            <div
                                class="flex items-center gap-1 text-[10px] text-amber-600 dark:text-amber-500 font-mono bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded w-fit border border-amber-100 dark:border-amber-800/50"
                            >
                                <span>Reasoning</span>
                            </div>
                        {/if}
                        {#if messageData.tool_calls && messageData.tool_calls.length > 0}
                            <div
                                class="flex items-center gap-1 text-[10px] text-purple-600 dark:text-purple-400 font-mono bg-purple-50 dark:bg-purple-900/20 px-1.5 py-0.5 rounded w-fit border border-purple-100 dark:border-purple-800/50"
                            >
                                <Wrench size={10} />
                                <span
                                    >{messageData.tool_calls.length} Tool Call{messageData
                                        .tool_calls.length > 1
                                        ? "s"
                                        : ""}</span
                                >
                    ...
                    {#if messageData.content}
                        <div class="truncate">
                            {messageData.content.substring(0, 100)}{messageData
                                .content.length > 100
                                ? "..."
                                : ""}
                        </div>
            ...
            {#if isOpen}
                <div
                    class="p-3 border-t border-gray-100 dark:border-gray-800"
                    transition:slide|local
                >
                    {#if messageData.reasoning}
                        <div
                            class="mb-3 bg-amber-50 dark:bg-amber-900/10 rounded-md border border-amber-100 dark:border-amber-800/30 overflow-hidden"
                        >
                            <div
                                ...
                                on:keydown={(e) =>
                                    e.key === "Enter" && toggleReasoning()}
                                role="button"
                                tabindex="0"
                            >
                                <div
                                    class:rotate-90={isReasoningOpen}
                                    class="transition-transform text-amber-500"
                                >
                                    <ChevronRight size={12} />
                                </div>
                                <div
                                    class="text-[10px] uppercase text-amber-600 dark:text-amber-500 font-bold flex items-center gap-1"
                                >
                            ...
                            {#if isReasoningOpen}
                                <div class="px-3 pb-3" transition:slide|local>
                                    <div
                                        class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 text-[11px] leading-relaxed border-l-2 border-amber-200 dark:border-amber-800 pl-2 ml-1"
                                    >
                    ...
                    {#if messageData.tool_calls && messageData.tool_calls.length > 0}
                        <div class="mb-3">
                            <div
                                class="text-[10px] uppercase text-purple-600 dark:text-purple-400 mb-1 font-bold flex items-center gap-1"
                            >
                            ...
                            <div class="space-y-2">
                                {#each messageData.tool_calls as call}
                                    <div
                                        class="bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800/30 rounded p-2 text-xs font-mono"
                                    >
                                        {#if call.function}
                                            <div
                                                class="font-bold text-purple-700 dark:text-purple-300 mb-1 flex items-center gap-2"
                                            >
                                                <span>{call.function.name}</span
                                                >
                                            </div>
                                            <div
                                                class="text-gray-600 dark:text-gray-400 whitespace-pre-wrap break-all"
                                            >
                    ...
                    {#if messageData.content}
                        <div
                            class="whitespace-pre-wrap text-gray-800 dark:text-gray-200 mb-2 font-sans"
                        >
                            {messageData.content}
                        </div>
                    {/if}
                    ...
                    {#if messageData.metadata && Object.keys(messageData.metadata).length > 0}
                        <div
                            class="mt-3 pt-2 border-t border-gray-100 dark:border-gray-800 border-dashed"
                        >
                            <div
                                class="text-[10px] uppercase text-gray-400 mb-1 font-bold flex items-center gap-1"
                            >
    ...
    {:else if isArray}
        <div>
            ...
            >
                ...
                {#if keyName}
                    <span class="font-medium" style="color: var(--vscode-link);"
                        >{keyName}:</span
                    >
                {/if}
                <span style="color: var(--vscode-secondary-fg);"
                    >Array({value.length})</span
                >
                {#if isMessageList}
                    <span
                        class="bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 px-1.5 rounded text-[10px] ml-2"
                        >Messages</span
                    >
                {/if}
            </div>
            ...
            {#if isOpen}
                <div
                    class="pl-4 border-l border-gray-200 dark:border-gray-800 ml-1 mt-1 space-y-1"
                    transition:slide|local
                >
                    {#each value as item, i}
                        <svelte:self
                            value={item}
                            keyName={isMessageList ? undefined : `${i}`}
                            level={level + 1}
                        />
                    {/each}
                </div>
