<script lang="ts">
    ...
    import { PlayCircle, Wrench, MessageSquare, Link, CheckCircle, ZoomIn, ZoomOut, Maximize, RefreshCw } from 'lucide-svelte';
    ...
    let links: { source: LayoutNode, target: LayoutNode, type: 'vertical' | 'horizontal' }[] = [];
    ...
    $: if (graphWidth && graphHeight && containerWidth && containerHeight && !isManualZoom) {
        fitToScreen();
    }
    ...
    function applyAutoLayout() {
        ...
        const logicalNodes = new Map<string, { id: string, name: string, spans: Span[] }>();

        if (viewMode === 'compact') {
            // Group by langgraph_node
            logicalNodes.set('Input', { id: 'logical-Input', name: 'Input', spans: [rootSpan] });
            chains.forEach(s => {
                if (s.id === rootSpan.id) return;
                const name = s.metadata?.langgraph_node || s.name;
                if (!logicalNodes.has(name)) {
                    logicalNodes.set(name, { id: `logical-${name}`, name, spans: [] });
                }
                logicalNodes.get(name)!.spans.push(s);
            });
            logicalNodes.set('Output', { id: 'logical-Output', name: 'Output', spans: [rootSpan] });

            logicalNodes.forEach(ln => {
                const w = measureWidth(ln.name);
                g.setNode(ln.id, { width: w, height: NODE_HEIGHT, label: ln.name });
            });

            // Transitions
            const actualChildren = chains
                .filter(s => s.id !== rootSpan.id && s.metadata?.langgraph_step !== undefined)
                .sort((a, b) => a.metadata.langgraph_step - b.metadata.langgraph_step);

            if (actualChildren.length > 0) {
                const first = actualChildren[0].metadata.langgraph_node || actualChildren[0].name;
                g.setEdge('logical-Input', `logical-${first}`);
                for (let i = 0; i < actualChildren.length - 1; i++) {
                    const from = actualChildren[i].metadata.langgraph_node || actualChildren[i].name;
                    const to = actualChildren[i+1].metadata.langgraph_node || actualChildren[i+1].name;
                    if (from !== to) g.setEdge(`logical-${from}`, `logical-${to}`);
                }
                const last = actualChildren[actualChildren.length - 1].metadata.langgraph_node || actualChildren[actualChildren.length - 1].name;
            ...
            g.setNode('marker-input', { width: measureWidth('Input'), height: NODE_HEIGHT, label: 'Input' });
            
            const sorted = chains
                .filter(s => s.id !== rootSpan.id)
                .sort((a, b) => (getStepIndex(a) || 0) - (getStepIndex(b) || 0));

            sorted.forEach(s => {
                g.setNode(s.id, { width: measureWidth(s.name), height: NODE_HEIGHT, label: s.name });
            });

            g.setNode('marker-output', { width: measureWidth('Output'), height: NODE_HEIGHT, label: 'Output' });
        ...
        g.nodes().forEach(v => {
            ...
            if (viewMode === 'compact') {
                ...
                const representative = ln.spans.find(s => s.status === 'error') || ln.spans[ln.spans.length - 1];
                allSpans = ln.spans;
                span = {
                    ...representative,
                    name: ln.name,
                    ...
                    metadata: {
                        ...
                        compact_count: ln.spans.length > 1 ? ln.spans.length : undefined
                    }
                };
            } else {
                if (v === 'marker-input') {
                    span = { ...rootSpan, name: 'Input', type: 'marker', id: 'marker-input' };
                } else if (v === 'marker-output') {
                    span = { ...rootSpan, name: 'Output', type: 'marker', id: 'marker-output' };
                } else {
            ...
            calculatedNodes.push({ span, allSpans, x, y, width: layoutData.width });
        ...
        const newLinks: { source: LayoutNode, target: LayoutNode, type: 'vertical' | 'horizontal' }[] = [];
        g.edges().forEach(e => {
    ...
    function handleNodeClick(e: MouseEvent, node: LayoutNode) {
        ...
        if (viewMode === 'compact' && node.allSpans && node.allSpans.length > 0) {
            // Cycle through spans
            const currentIndex = node.allSpans.findIndex(s => s.id === selectedId);
    ...
    function getLinkPath(link: { source: LayoutNode, target: LayoutNode, type: 'vertical' | 'horizontal' }) {
        ...
        const getAnchor = (node: LayoutNode, isSource: boolean) => {
            ...
            const sideLinks = links.filter(l => {
                ...
                if (l.source.span.id === node.span.id) {
                    const lside = (Math.abs(ldx) / node.width > Math.abs(ldy) / NODE_HEIGHT) ? (ldx > 0 ? 'right' : 'left') : (ldy > 0 ? 'bottom' : 'top');
                    return lside === side;
            ...
            sideLinks.sort((a, b) => {
                const aOther = a.source.span.id === node.span.id ? a.target : a.source;
                const bOther = b.source.span.id === node.span.id ? b.target : b.source;
                return (side === 'top' || side === 'bottom') ? (aOther.x - bOther.x) : (aOther.y - bOther.y);
            });
            ...
            return {
                x: (side === 'right') ? nx + nw : (side === 'left' ? nx : nx + nw / 2 + offset),
                y: (side === 'bottom') ? ny + NODE_HEIGHT : (side === 'top' ? ny : ny + NODE_HEIGHT / 2 + offset),
                side,
                index
            };
        };
        ...
        const radius = 12;
        const horizontalRouting = (start.side === 'left' || start.side === 'right');

        // LANE ROUTING: Offset the mid-segments to prevent overlapping lines
        const LANE_WIDTH = 8;
        const laneOffset = ( (start.index || 0) + (end.index || 0) ) * (LANE_WIDTH / 2);
        ...
        if (horizontalRouting) {
            ...
            if (Math.abs(ey - start.y) < radius * 2) return `M ${start.x} ${start.y} L ${ex} ${ey}`;
            ...
            if (Math.abs(ex - start.x) < radius * 2) return `M ${start.x} ${start.y} L ${ex} ${ey}`;
                ...
                <g transform="translate({PADDING}, {PADDING})">
                    ...
                    {#each nodes as node}
                        <g 
                            ...
                            class="cursor-pointer transition-opacity hover:opacity-90 outline-none {isDragging && draggedNode === node ? 'cursor-grabbing' : 'cursor-grab'}"
